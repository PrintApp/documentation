...
| `app:display:changed`       | -     | Only in the Dialog [Display Mode](/configuration/display-modes): dispatched when the customer expands the editor to full window or shrinks it back.<br/>A data object of `{ expanded: Boolean }` is passed to your function. |
