...
<Accordion title="Printed Sides">
The Printed Sides connector reports how many pages, or sides, of the design actually carry a print. A side the customer leaves blank is not counted. This is useful for apparel and other multi-side products where the first print is included and each additional side costs extra: a T-shirt with front, back and two sleeves reports `1` while only the front is designed, and `2` the moment something is placed on the back. The value is sent to your input field as the customer works, so the price can update before they add to cart.<br/>
Objects that belong to the design itself (locked template graphics and reference guides) do not count as a print, only what the customer places or edits.<br/>
This connector only sends data from the editor to your page. The target must be a field that takes a number: a number input, or a dropdown or [Print Options](/print-options/introduction) choice field whose choices are named `1`, `2`, `3` and so on. A choice field with options like "Single-sided / Double-sided" will not be updated.
</Accordion>
